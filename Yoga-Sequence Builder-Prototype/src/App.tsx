...
import { Dumbbell, ListOrdered, BookOpen, LogOut, User } from 'lucide-react';
...
export default function App() {
  ...
  const handleViewAsGuest = () => {
    setIsGuestViewing(true);
  };

  const handleBackToAuth = () => {
    setIsGuestViewing(false);
  };

  // Show guest profile view
  if (isGuestViewing && !isAuthenticated) {
    return <PublicProfile onBack={handleBackToAuth} />;
  }

  // Show auth screen if not authenticated
  if (!isAuthenticated) {
    return <Auth onLogin={handleLogin} onViewAsGuest={handleViewAsGuest} />;
  }
            ...
            <Tabs defaultValue="library" className="w-full">
              <TabsList className="grid w-full grid-cols-3">
                ...
                <TabsTrigger value="builder">
                  ...
                  <span className="hidden sm:inline">Sequence Builder</span>
                </TabsTrigger>
                <TabsTrigger value="sequences">
                  <BookOpen className="h-4 w-4 sm:mr-2" />
                  <span className="hidden sm:inline">Sequence Library</span>
                </TabsTrigger>
              </TabsList>
